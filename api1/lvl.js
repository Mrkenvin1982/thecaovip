...
Phones.findById(objb.id).then(function(objc) {
   if (objc.status==1) {
            objc.update({
                status : 2
            }).then(function() {
    ...
    //nếu là trả trước thì nạp qua sim, cần lấy port chưa nạp quá 5 lần
      ...
      var portRun = [];
        ...
        for(var i = 0; i < ports.length; i++) {
            ...
            if(ports[i].status === 0 && ports[i].fail < 3&& ports[i].type === 1) {
                portRun.push(i);
...
//nếu chạy được tới đây là đã có port
            var randIndex =getRandomInt(0,(portRun.length-1));
            var portIndex = portRun[randIndex];
...
var tus =1;
 ...
 console.log('GD trung lap');
                res.json({status : false, code : 30, msg : 'Hệ thống bận, vui lòng thử lại '});
                return;
}
})
